"""
Population Attack (attack-p) implementation within ml_privacy_meter.

Uses the RMIA trained models. Scores each sample by its empirical CDF rank
within the population distribution: the fraction of population samples whose
confidence is lower than the sample's confidence.

Key points:
- Requires population signals (same data source used by RMIA).
- Reuses RMIA signals and population signals from disk when available.
- Uses identical dataset splits as RMIA for fair comparison.
"""
...
def main(dataset_name: str = "locations", model_name: str = None, mode: str = "load", seed: int = None):
    ...
    log_dir = os.path.join(base_log_dir, "attack_p")
    rmia_log_dir = os.path.join(base_log_dir, "rmia")
    configs["run"]["log_dir"] = log_dir
    configs["audit"]["algorithm"] = "POPULATION"
    # Population attack only needs the target model signal — no reference models.
    ...
    # Population attack only needs the target model — one pair is sufficient.
    num_model_pairs = max(math.ceil(num_experiments / 2.0), 1)

    # Population attack always reuses RMIA models — no separate training needed.
    ...
    # Trim to just the target models — paired model not needed for population attack.
    target_models_list = models_list[:num_experiments]
    auditing_membership = auditing_membership[:num_experiments, :]

    # get_model_signals derives the cache filename from the algorithm name:
    # algorithm="POPULATION" -> looks for "population_signals.npy" / "population_signals_pop.npy"
    # The auditing signals are identical to RMIA's (same model, same data), so reuse them.
    rmia_signal_path = os.path.join(rmia_log_dir, "signals", "rmia_signals.npy")
    sig_cache_path = os.path.join(directories["signal_dir"], "population_signals.npy")
    pop_cache_path = os.path.join(directories["signal_dir"], "population_signals_pop.npy")

    if not os.path.exists(sig_cache_path) and os.path.exists(rmia_signal_path):
        rmia_sigs = np.load(rmia_signal_path)
        if rmia_sigs.shape[0] == len(auditing_dataset) and rmia_sigs.shape[1] >= num_experiments:
            logger.info("Reusing RMIA signals for population attack (no inference needed)")
            os.makedirs(directories["signal_dir"], exist_ok=True)
            np.save(sig_cache_path, rmia_sigs[:, :num_experiments])

    # Population signals (held-out 25%) are also identical to RMIA's — reuse.
    rmia_pop_path = os.path.join(rmia_log_dir, "signals", "rmia_signals_pop.npy")
    if not os.path.exists(pop_cache_path) and os.path.exists(rmia_pop_path):
        rmia_pop_sigs = np.load(rmia_pop_path)
        logger.info("Reusing RMIA population signals for population attack")
        os.makedirs(directories["signal_dir"], exist_ok=True)
    ...
    # Population signals are required for the population attack.
    # Build a minimal population dataset (same as RMIA: the held-out 25%).
    population_size = len(y) - training_size
    population_data = TabularDataset(X[training_size:], y[training_size:])

    logger.info("Preparing population signals")
    population_signals = get_model_signals(
        target_models_list, population_data, configs, logger, is_population=True
    )
    ...
    logger.info("Population attack runtime: %0.5f seconds", time.time() - start_time)


if __name__ == "__main__":
    import argparse

    parser = argparse.ArgumentParser(description="Run population membership inference attack.")
    ...
    ensure_dataset_ready(dataset_name=args.dataset, model_name=args.model, algorithm="POPULATION", skip_if_exists=args.skip_config)
