#!/usr/bin/env python3
"""
Attention-based Membership Inference Analysis (AMIA) for TabICL
===============================================================

Research question
-----------------
Why is RMIA disproportionately effective against TabICL compared to classical
models such as RF or LightGBM?

Hypothesis
----------
TabICL processes tabular data through a three-stage in-context Transformer.
Training members are permanently embedded in the ICL context window, so the
model can attend to them more strongly and sharply than to non-members.  This
creates a measurable side-channel: members produce more *concentrated* attention
weights in the ICL stage, which correlates directly with the higher RMIA scores
they receive.

TabICL architecture (three stages)
------------------------------------
  Stage 1 — ColEmbedding  (SetTransformer, 3 induced-attention blocks)
      Computes per-column distribution-aware embeddings.
      Induced attention can attend over training-row statistics for each feature.
      It is not the AMIA signal because it is not the final query-to-context
      in-context learning attention.

  Stage 2 — RowInteraction  (Transformer, 3 blocks)
      Captures feature interactions *within* each row.
      Self-attention on features: q_len = k_len = n_features + num_cls.
      Last block is asymmetric: q = 4 CLS tokens, k/v = full feature sequence.
      Operates per-sample in parallel; no cross-sample signal.

  Stage 3 — ICLearning  (Transformer, 12 blocks)  <- MIA-relevant
      Query  : ALL positions  (q_len = train_size + test_size)
      Key/Val: ONLY training  (k_len = train_size)
      Members are IN the key/value set; non-members are not.
      The exact full-context shape identifies these calls.

SDPA call classification
------------------------
The hook intercepts F.scaled_dot_product_attention.  Inside
sdpa_with_flattened_batch (attention.py) tensors arrive as
(flat_batch, n_heads, seq_len, head_dim).

  is_icl  :  k_len == n_context AND q_len == n_context + batch_chunk
              ColEmbedding's induced attention ALSO has k_len == n_context (it restricts
              inducing-point keys to training samples).  Matching the exact ICL
              full-context length avoids treating induced attention as AMIA signal.
  ignored :  everything else (col induced, row self-attention)

TabICL default configuration (used in ml_privacy_meter)
---------------------------------------------------------
  TabICLClassifier(device=...) -- no KV cache by default
  icl_num_blocks=12, icl_nhead=8
  row_num_blocks=3,  row_nhead=8,  row_num_cls=4
  col_num_blocks=3,  col_nhead=8,  col_num_inds=128
  n_estimators=8  (8 ensemble members processed in one model_.forward call)

Signals extracted (ICL attention only)
----------------------------------------
  Per-call, per-head:
    max_attn    : max attention weight across keys -- concentration proxy
    neg_entropy : sum(w * log w) -- more peaked = less negative
    argmax      : key position with highest attention weight

  The hook captures 12 x n_ensemble ICL calls per predict_proba call.
  _group_by_layer averages ensemble members at the same layer depth,
  collapsing to exactly 12 layer signals.

  NOTE -- Key positions 0..n_context-1 map directly to training samples.
  No thinking rows exist in TabICL.  TabPFN may prepend checkpoint-dependent
  thinking rows; TabICL key positions 0..n_context-1 are all real context rows.

Capture safeguards
------------------
  TabICL is not treated as TabPFN.  AMIA temporarily disables TabICL's internal
  Flash Attention 3 and KV/repr cache during signal extraction so the standard
  full train+test ICL attention is visible to the Python SDPA hook.
...
Usage examples
--------------
    uv run run_attacks/amia/amia_tabicl.py --dataset locations
    uv run run_attacks/amia/amia_tabicl.py --dataset purchases10 --gpu 0
    uv run run_attacks/amia/amia_tabicl.py --dataset locations --model-idx 2
"""
...
class SDPACapture:
    ...
    per-call, per-head attention statistics for TabICL's ICL stage.

    Call classification
    -------------------
    is_icl : k_len == n_context AND q_len == n_context + chunk_size
        ICL cross-attention: all T positions query, only training as K/V.
        ColEmbedding's induced attention also has k_len == n_context when
        train_size is restricted.  The exact full-context q_len match excludes
        those calls without assuming anything about the number of inducing points.

    All other calls (col induced attention, row self-attention) are ignored.

    Per-head statistics (ICL calls only)
    --------------------------------------
    ...
    For ICL attention the query includes both training and test positions
    (q_len = n_context + chunk_size); only the last chunk_size rows are kept.
    ...
    def __enter__(self):
        ...
        def _hook(query, key, value,
            ...
            # True no-cache TabICL ICL calls have q_len = n_context + chunk
            # and k_len = n_context.  ColEmbedding can also have k_len equal to
            # the training size, so do not use a loose q_len > k_len test here.
            if k_len == n_context and q_len == n_context + chunk_size:
                with torch.no_grad():
                    d  = query.shape[-1]
                    ...
                    w = torch.softmax(sc, dim=-1).cpu()   # (*, n_heads, q_len, k_len)
                    del sc
                    ...
                    call_idx = icl_call_counter[0]
                    icl_call_counter[0] += 1
...
def tabicl_capture_runtime(model, logger=None):
    """Force TabICL through the full-context Python SDPA path while capturing.

    TabICL differs from TabPFN here: its sklearn wrapper can use an internal
    KV/repr cache and its InferenceManager can enable Flash Attention inside the
    forward call.  Both are valid for normal prediction, but AMIA needs the
    uncached train+test ICL attention weights to be visible to the Python SDPA
    hook.
    """
    changes = []
    ...
    set_attr(model, "use_fa3", False)

    cfg = getattr(model, "inference_config_", None)
    for cfg_name in ("COL_CONFIG", "ROW_CONFIG", "ICL_CONFIG"):
        set_attr(getattr(cfg, cfg_name, None), "use_fa3", False)

    raw = getattr(model, "model_", None)
    for mgr_path in (
        ("col_embedder", "inference_mgr"),
        ("row_interactor", "inference_mgr"),
        ("icl_predictor", "inference_mgr"),
    ):
        mgr = raw
        for part in mgr_path:
            mgr = getattr(mgr, part, None)
            if mgr is None:
                break
        set_attr(mgr, "use_fa3", False)

    ensemble = getattr(model, "ensemble_generator_", None)
    has_training_data = ensemble is not None and getattr(ensemble, "X_", None) is not None
    if has_training_data and getattr(model, "model_kv_cache_", None) is not None:
        set_attr(model, "model_kv_cache_", None)
        if logger is not None:
            logger.info(
                "Temporarily disabled TabICL KV/repr cache for AMIA capture; "
                "using full train+test context so q_len = n_context + batch_size."
            )

    try:
        yield
    finally:
        for obj, name, old_value in reversed(changes):
...
def compute_roc(scores: np.ndarray, labels: np.ndarray):
    """Return (fpr, tpr, auc) for a binary classifier. Returns 0.5 AUC on degenerate input."""
...
def extract_attention_signals(model, X_pool: np.ndarray,
    ...
    """
    Run inference over all pool samples and collect per-call, per-head ICL signals.

    Returns
    -------
    row_max_all : (n_pool, n_icl_calls, n_heads) float32
    row_ent_all : (n_pool, n_icl_calls, n_heads) float32
    row_arg_all : (n_pool, n_icl_calls, n_heads) int32
    All return None on failure.
    """
    # tabicl_capture_runtime uses setattr/getattr to toggle model state.  If
    # model is a wrapper (e.g. KAnonTabFMWrapper) __getattr__ delegates reads
    # to the underlying model, but setattr creates instance attributes on the
    # wrapper, not on self.model.  Unwrap one layer so tabicl_capture_runtime
    # operates on the raw TabICL object directly.
    capture_model = model
    while hasattr(capture_model, "model") and not hasattr(type(capture_model), "fit"):
        capture_model = capture_model.model
    n_pool = len(X_pool)
    logger.info("Effective TabICL AMIA attention extraction batch_size=%d", batch_size)

    # TabICL may use Flash Attention 3 in sdpa_with_flattened_batch, which bypasses
    # torch.nn.functional.scaled_dot_product_attention and therefore our hook.
    try:
    ...
    # Guard against torch.compile/fused paths bypassing the Python SDPA hook.
    try:
    ...
    for batch_start in range(0, n_pool, batch_size):
        ...
        fa_ctx = flash_attn3_toggle(False) if flash_attn3_toggle is not None else contextlib.nullcontext()
        with tabicl_capture_runtime(capture_model, logger):
            with ctx:
                with fa_ctx:
                    try:
        ...
        icl_calls = [r for r in ctx.records if r["type"] == "icl"]

        if batch_start == 0:
            n_icl_calls_ref = len(icl_calls)
            n_heads_ref     = icl_calls[0]["n_heads"] if icl_calls else 1
            logger.info(
                "SDPA calls: %d ICL cross-attention (n_heads=%d); "
                "TabICL full-context rule: k_len == n_context(%d), "
                "q_len == n_context + batch_chunk(%d)",
                n_icl_calls_ref, n_heads_ref, n_context, n_context + chunk,
            )
            if n_icl_calls_ref == 0:
                logger.error(
                    "No TabICL ICL attention calls captured in first batch; "
                    "expected calls with k_len == n_context(%d) and q_len == %d.",
                    n_context,
                    n_context + chunk,
                )
                return None, None, None
        elif len(icl_calls) != n_icl_calls_ref:
            new_count = len(icl_calls)
            if new_count < n_icl_calls_ref:
                if new_count == 0:
                    logger.error(
                        "ICL attention calls dropped to zero at batch %d-%d; aborting extraction.",
                        batch_start, batch_end,
                    )
                    return None, None, None
                logger.warning(
                    "ICL-call count dropped from %d to %d -- trimming all %d collected batches",
                    n_icl_calls_ref, new_count, len(row_max_batches),
                )
                ...
                n_icl_calls_ref = new_count
            else:
                logger.warning(
                    "ICL-call count changed: expected %d, got %d -- truncating",
                    n_icl_calls_ref, len(icl_calls),
                )
                icl_calls = icl_calls[:n_icl_calls_ref]

        if not icl_calls:
            logger.error(
                "No ICL attention calls captured at batch %d-%d; aborting extraction.",
                batch_start, batch_end,
            )
            return None, None, None

        rm = np.stack([r["max_attn"][:, :chunk]    for r in icl_calls], axis=0).transpose(2, 0, 1)
        re = np.stack([r["neg_entropy"][:, :chunk] for r in icl_calls], axis=0).transpose(2, 0, 1)
        ra = np.stack([r["argmax"][:, :chunk]      for r in icl_calls], axis=0).transpose(2, 0, 1)
...
def plot_distributions(signals_dict: dict, mem_mask: np.ndarray,
    ...
    """Plot 01: KDE distributions of ICL attention signals split by membership."""
    labels = ["ICL attention\n(max_attn)", "ICL attention\n(neg_entropy)"]
...
def plot_attention_vs_rmia(signals_dict: dict, rmia_scores: np.ndarray,
    ...
    """Plot 02: Scatter of ICL max attention vs RMIA score, coloured by membership."""
    fig, ax = plt.subplots(figsize=(7, 4.5))
    fig.suptitle(
        f"ICL attention concentration vs RMIA score -- {dataset_name} / {model_name}\n"
    ...
    try:
        ...
        pass
    ax.set_xlabel("ICL attention (max_attn, head+layer avg)", fontsize=9)
...
def plot_roc_comparison(signals_dict: dict, rmia_scores: np.ndarray,
                        mem_mask: np.ndarray, rep_dir: str,
                        dataset_name: str, model_name: str):
    """Plot 03: ROC curves for ICL attention vs RMIA, full and low-FPR zoom."""
...
# TabICL: 12 ICL blocks, 8 heads; 1 SDPA call per block per ensemble member.
# With n_estimators=8, expect 12 x 8 = 96 ICL calls per predict_proba.
# _group_by_layer averages ensemble members at the same depth -> 12 layer signals.
_TABICL_ICL_LAYERS = 12


def _group_by_layer(arr: np.ndarray, n_layers: int = _TABICL_ICL_LAYERS) -> np.ndarray:
    """
    Collapse raw ICL SDPA calls into encoder layers by averaging within each group.

    With n_estimators=8 and 12 ICL layers: 96 raw calls grouped into 12 layers
    (8 ensemble calls per layer averaged) -> (n_pool, n_layers, n_heads).

    Parameters
    ----------
    arr      : (n_pool, n_raw_calls, n_heads)
    n_layers : ICL layers to group into (default: _TABICL_ICL_LAYERS=12)
    """
...
def plot_layer_auc(row_max_all: np.ndarray, row_ent_all: np.ndarray,
                   mem_mask: np.ndarray, rep_dir: str,
                   dataset_name: str, model_name: str):
    """Plot 04: Per-layer AUC of ICL max_attn and neg_entropy."""
    n_layers = _TABICL_ICL_LAYERS
    rm = _group_by_layer(row_max_all, n_layers)
    ...
    ax.set_title(
        f"Membership discriminability per ICL layer -- {dataset_name} / {model_name}\n"
        f"{n_layers} ICL layers ({calls_per_layer} ensemble calls/layer averaged)",
        fontsize=10, fontweight="bold",
    )
...
def plot_entropy_divergence(row_ent_all: np.ndarray, mem_mask: np.ndarray,
    ...
    ax.set_title(
        f"Attention entropy divergence per ICL layer -- {dataset_name} / {model_name}\n"
        f"{n_layers} ICL layers ({calls_per_layer} ensemble calls/layer averaged)",
        fontsize=10, fontweight="bold",
    )
    ax.legend(fontsize=9);  ax.grid(True, alpha=0.3)
...
def plot_layer_head_heatmap(row_max_all: np.ndarray, mem_mask: np.ndarray,
                            rep_dir: str, dataset_name: str, model_name: str):
    """Plot 06: AUC heatmap over (ICL layer x attention head)."""
    n_layers = _TABICL_ICL_LAYERS
    ...
    for l in range(n_layers):
        for h in range(n_heads):
            _, _, a = compute_roc(rm[:, l, h], labels_int)
            auc_mat[l, h] = a
    ...
    ax.set_ylabel("ICL layer", fontsize=9)
    ax.set_title(
        f"Membership AUC per (ICL layer x head) -- {dataset_name} / {model_name}",
        fontsize=10, fontweight="bold",
    )
    fig.tight_layout()
...
def plot_argmax_analysis(row_arg_all: np.ndarray, mem_mask: np.ndarray,
    ...
    Unlike TabPFN, TabICL has no thinking rows.  Key positions 0..n_context-1
    map directly to training samples; no masking is required.
    """
    n_layers = _TABICL_ICL_LAYERS
    n_pool, n_raw, n_heads = row_arg_all.shape
    calls_per_layer = n_raw // n_layers
    trimmed  = row_arg_all[:, :calls_per_layer * n_layers, :]
    flat     = trimmed.reshape(n_pool, n_layers, -1).astype(np.int32)
    ...
    fig.suptitle(
        ...
        "Fraction of (ICL layer x head) pairs attending to the same training-sample position\n"
    ...
    ax.set_xlabel("Mode fraction (all ICL layers x heads)", fontsize=8)
    ax.set_ylabel("Density", fontsize=8)
    ...
    ax.set_xlabel("ICL layer", fontsize=8)
...
def plot_entropy_slope(row_ent_all: np.ndarray, mem_mask: np.ndarray,
    ...
    fig, axes = plt.subplots(1, 2, figsize=(12, 4.5))
    fig.suptitle(
        f"Entropy slope across ICL layers -- {dataset_name} / {model_name}\n"
    ...
    ax.set_xlabel("ICL layer transition  (L -> L+1)", fontsize=8)
...
def plot_feature_block_variance(row_max_all: np.ndarray, mem_mask: np.ndarray,
                                rep_dir: str, dataset_name: str, model_name: str):
    """
    Plot 09: Variance of max_attn across ensemble members within each ICL layer.

    For TabICL with n_estimators=8, each ICL layer L has 8 SDPA calls (one per
    ensemble member).  Variance across the 8 calls measures how differently each
    ensemble view concentrates attention.
    """
    n_layers = _TABICL_ICL_LAYERS
    ...
    grouped_hm = grouped.mean(axis=3)    # (n_pool, n_layers, calls_per_layer)
    block_var  = grouped_hm.var(axis=2)  # (n_pool, n_layers)
    ...
    total_var = block_var.mean(axis=1)
    _, _, auc_var = compute_roc(total_var, labels_int)

    fig, axes = plt.subplots(1, 2, figsize=(12, 4.5))
    fig.suptitle(
        f"Ensemble variance per ICL layer -- {dataset_name} / {model_name}\n"
        f"Variance of max_attn across {calls_per_layer} ensemble members within each ICL layer",
        fontsize=10, fontweight="bold",
    )
    ...
    ax.set_xlabel("ICL layer", fontsize=8)
    ax.set_ylabel("Mean intra-layer ensemble variance", fontsize=8)
    ax.set_title("Per-layer ensemble variance", fontsize=9, fontweight="bold")
    ax.set_xticks(x);  ax.set_xticklabels([f"L{i}" for i in x], fontsize=7)
    ax.legend(fontsize=8);  ax.grid(True, alpha=0.25)
    ...
    for mask, name, color in [
        ...
        try:
            ...
            ax.hist(vals, bins=25, alpha=0.4, color=color, label=name, density=True)
    ax.set_title(f"Total ensemble variance  (AUC={auc_var:.3f})", fontsize=9, fontweight="bold")
    ax.set_xlabel("Mean variance across all ICL layers", fontsize=8)
    ax.set_ylabel("Density", fontsize=8)
    ax.legend(fontsize=8);  ax.grid(True, alpha=0.25)
...
def run_amia_pipeline(
    ...
    """Extract TabICL ICL attention signals and generate AMIA plots.

    This mirrors ``amia_tabpfn.run_amia_pipeline`` so defense evaluation can
    dispatch by model family.  Even though the captured call is TabICL's ICL
    attention, the saved AMIA signal names are the shared ``row_*`` names used
    by all AMIA backends.
    """
    del max_row_calls, max_col_calls  # API compatibility with TabPFN pipeline
    ...
    if os.path.exists(cache):
        npz = np.load(cache)
        row_max_all = npz["row_max_all"] if "row_max_all" in npz else npz["icl_max_all"]
        row_ent_all = npz["row_ent_all"] if "row_ent_all" in npz else npz["icl_ent_all"]
        row_arg_all = (
            npz["row_arg_all"] if "row_arg_all" in npz
            else npz["icl_arg_all"] if "icl_arg_all" in npz
            else None
        )
        if row_max_all.shape[0] != len(mem):
            msg = (
                f"Cached TabICL AMIA signals have {row_max_all.shape[0]} rows, "
            ...
            logger.info("Loaded cached TabICL signals from %s  shape=%s", cache, row_max_all.shape)

    if not os.path.exists(cache) or row_arg_all is None:
        logger.info("Extracting TabICL ICL attention signals")
        ...
        logger.info("Cached TabICL signals to %s  shape=%s", cache, row_max_all.shape)

    logger.info("Signal shapes: row_max_all=%s", row_max_all.shape)
    ...
    logger.info("Per-signal AUC and Cohen's d (ICL head/layer averaged):")
    scalars: dict = {}
...
def main(dataset_name: str, model_name: str, gpu, batch_size: int,
    ...
    if skip_existing and os.path.exists(summary_csv) and os.path.exists(cache):
        logger.info(
            "--skip-existing: AMIA outputs already exist for %s/%s seed=%s; skipping.",
            dataset_name,
            model_name,
            seed,
        )
    ...
    if plots_only:
        ...
        context_size = n_members   # TabICL k_len = training set size
        logger.info(
            "Target model %d: %d members, %d non-members in RMIA audit pool",
            model_idx, n_members, n_pool - n_members,
        )
        logger.info(
            "Reconstructed TabICL AMIA audit pool from seeded RMIA memberships: "
        ...
        if rmia_signals.shape[0] != n_pool or auditing_membership.shape[1] != n_pool:
            raise ValueError(
                "RMIA signal rows do not match reconstructed TabICL AMIA audit pool: "
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Attention-based explanation of RMIA effectiveness on TabICL.",
    ...
    if args.seeds is not None:
        ...
        for seed in seeds:
            if args.skip_existing:
                _base  = os.path.join("ml_privacy_meter", "logs",
                                      args.dataset, "tabicl", f"seed{seed}")
                ...
                if os.path.exists(_cache) and os.path.exists(_csv):
                    print(f"[SEED] TabICL AMIA seed={seed} already done, skipping.")
                    continue
            cmd = [
                sys.executable,
                __file__,
            ...
            print(f"[SEED] Running TabICL AMIA seed={seed}")
        ...
        if failures:
            raise SystemExit(1)
        print(f"[SEED] Completed TabICL AMIA for seeds: {seeds}")
        raise SystemExit(0)
