...
def _kanon_keys_indexed(K: torch.Tensor, k: int) -> torch.Tensor:
    """K-anonymity grouped by sample index (sample i → group i//k).

    The grouping is IDENTICAL across all feature groups, heads, and layers
    because it depends only on position in the sequence, not on key values.
    This makes the k-anonymity mask consistent when attention is averaged
    over feature groups, which is the correct behaviour for TabPFN.
    K : (..., N, d_head).
    """
    *leading, N, D = K.shape
    if N < k or k <= 1:
    ...
    pad = (k - N % k) % k
    if pad > 0:
        K_pad = torch.cat([K_flat, K_flat[:, -1:].expand(-1, pad, -1)], dim=1)
    else:
        K_pad = K_flat
    n_groups  = (N + pad) // k
    centroids = K_pad.reshape(BH, n_groups, k, D).mean(dim=2)        # (BH, G, D)
    c_per_pos = centroids.unsqueeze(2).expand(-1, -1, k, -1).reshape(BH, N + pad, D)[:, :N]
    return c_per_pos.reshape(*leading, N, D)


# Alias used by the capture block — default to index-based (globally consistent).
_kanon_keys_for_capture = _kanon_keys_indexed
...
class MultiHeadAttention(Attention):
    ...
    def __init__(  # noqa: PLR0913
        ...
        *,
        ...
        # When set to a list, _compute appends (batch, nhead, seq_q, seq_kv)
        # attention-weight tensors here.  Used by external analysis (e.g. AMIA).
        self._attn_capture: list | None = None
        self._attn_kanon_k: int | None = None        # [MIA-ADDED] k for k-anon inside capture
        self._key_capture:  list | None = None        # [MIA-ADDED] raw key vectors for PCA
        self._attn_defense_dropout_p: float | None = None  # [MIA-ADDED] inference-time dropout
    ...
    def _compute(
        ...
        *,
        ...
        # [MIA-ADDED] Route capture based on cross/self flag set by layer.py.
        # _attn_is_cross=True  → cross-attention: always capture (bypass seqlen guard).
        # _attn_is_cross=False → self-attention:  suppress capture.
        # _attn_is_cross=None  → no flag: fall back to seqlen_q < seqlen_kv guard.
        _capture_raw  = getattr(self, "_attn_capture", None)
        _attn_is_cross = getattr(self, "_attn_is_cross", None)
        if _attn_is_cross is False:
            effective_capture = None          # suppress self-attention capture
            force_capture     = False
        elif _attn_is_cross is True:
            effective_capture = _capture_raw  # cross: capture regardless of seqlens
            force_capture     = True
        else:
            effective_capture = _capture_raw  # legacy: guard decides inside compute_attention_heads
            force_capture     = False
        _kanon_k  = getattr(self, "_attn_kanon_k", None)                                        # [MIA-ADDED]
        _key_cap  = getattr(self, "_key_capture", None) if effective_capture is not None else None  # [MIA-ADDED]
        _def_dp   = getattr(self, "_attn_defense_dropout_p", None)                              # [MIA-ADDED]
        # [MIA-ADDED] Apply defense dropout only to cross-attention.
        # For self-attention, keep the model's native dropout setting.
        if _def_dp is not None:
            if _attn_is_cross is True:
                effective_dp = _def_dp
            elif _attn_is_cross is False:
                effective_dp = self.dropout_p
            else:
                # Legacy path without explicit cross/self tag.
                # Fall back to shape-based cross-attention detection.
                effective_dp = _def_dp if q.shape[-3] < k.shape[-3] else self.dropout_p
        else:
            effective_dp = self.dropout_p
        attention_head_outputs = MultiHeadAttention.compute_attention_heads(
            ...
            kv,
            qkv,
            effective_dp,                  # [MIA-ADDED] may be overridden by defense
            self.softmax_scale,
            _capture=effective_capture,        # [MIA-ADDED]
            _force_capture=force_capture,      # [MIA-ADDED]
            _kanon_k=_kanon_k,                 # [MIA-ADDED]
            _key_capture=_key_cap,             # [MIA-ADDED]
        )
        return torch.einsum(
            "... h d, h d s -> ... s",
    ...
    def compute_attention_heads(
        ...
        softmax_scale: float | None = None,
        *,
        _capture: list | None = None,       # [MIA-ADDED]
        _force_capture: bool = False,       # [MIA-ADDED] bypass seqlen guard when True
        _kanon_k: int | None = None,        # [MIA-ADDED] apply k-anon to keys inside capture
        _key_capture: list | None = None,   # [MIA-ADDED] collect raw key vectors for PCA
    ) -> torch.Tensor:
        assert (k is None) == (v is None)
        ...
        # [MIA-ADDED] Capture cross-attention weights (test→train) for AMIA.
        # _force_capture=True: layer.py tagged this as cross-attention → always capture.
        # Fallback guard seqlen_q < seqlen_kv handles legacy calls without the tag
        # (skips train→train self-attention which would produce a huge square matrix).
        if _capture is not None and (_force_capture or seqlen_q < _seqlen_kv):  # [MIA-ADDED]
            with torch.no_grad():  # [MIA-ADDED]
                k_cap = MultiHeadAttention.broadcast_kv_across_heads(k, share_kv_across_n_heads)  # [MIA-ADDED]
                # [MIA-ADDED] Optionally apply k-anonymity to keys before computing
                # attention weights, so the captured signal reflects the defended view.
                # k_cap shape: (batch, seqlen_kv, nhead, d_k) → transpose to apply kanon.
                if _kanon_k is not None and _kanon_k > 1:                                        # [MIA-ADDED]
                    k_cap_t = k_cap.transpose(1, 2)                                              # [MIA-ADDED] (batch, nhead, N, d_k)
                    k_cap_t = _kanon_keys_for_capture(k_cap_t, _kanon_k)                        # [MIA-ADDED]
                    k_cap   = k_cap_t.transpose(1, 2)                                            # [MIA-ADDED] back to (batch, N, nhead, d_k)
                if _key_capture is not None:                                                     # [MIA-ADDED]
                    _key_capture.append(k_cap.transpose(1, 2).mean(dim=0))                      # [MIA-ADDED] (nhead, N, d_k) on GPU
                scale_val = (d_k ** -0.5) if softmax_scale is None else float(softmax_scale)  # [MIA-ADDED]
                attn_w = torch.softmax(  # [MIA-ADDED]
                    torch.matmul(  # [MIA-ADDED]
                        q.transpose(1, 2).float(),  # (batch, nhead, seqlen_q, d_k)  # [MIA-ADDED]
                        k_cap.transpose(1, 2).float().transpose(-2, -1),  # (batch, nhead, d_k, seqlen_kv)  # [MIA-ADDED]
                    ) * scale_val,  # [MIA-ADDED]
                    dim=-1,  # [MIA-ADDED]
                )  # [MIA-ADDED]
                if dropout_p is not None and dropout_p > 0.0:                              # [MIA-ADDED]
                    attn_w = torch.nn.functional.dropout(attn_w, p=dropout_p, training=True)  # [MIA-ADDED]
                _capture.append(attn_w.mean(dim=0))  # (nhead, seqlen_q, seqlen_kv) — stays on GPU  # [MIA-ADDED]
